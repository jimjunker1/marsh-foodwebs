# Food web functions
# the basis for this code was written by Owen Petchey, and the original can be found at https://github.com/opetchey/ttl-resources/blob/master/food_web/FoodWebFunctions.r

# code has been slightly modified in places by Justin Pomeranz to carry out analyses in Pomeranz et al. In Press. Inferring predator-prey interactions in food webs. Methods in Ecology and Evolution. 

# Plot.matrix2
# modified code from Petchey to allow for different point characers
# modified by Pomeranz july 2017
Plot.matrix2 <- function(web, title=" ",
                         point.cex=0.5,
                         trait.cex=1,
                         diag.line=T,
                         traits=F,
  ...
  plot(consumer, resource, pch=sp.pt.ch, type="n", cex=0.1,
       ann=F, axes=F,
  ...
  if(length(traits)==1)
    points(web.list[,2], S+1-as.numeric(web.list[,1]),
           type="p", pch=sp.pt.ch, cex=point.cex, col=pt.col)
  if(length(traits)==length(web)){
    ...
    if(point.cex>trait.cex){
      ...
      points(rep(1:S, each=S), rep(S:1, times=S), 
             pch=sp.pt.ch, cex=trait.cex, col=colours.to.use[integer.traits])
    }
    if(point.cex<trait.cex){
      points(rep(1:S, each=S), rep(S:1, times=S), 
             pch=sp.pt.ch, cex=trait.cex, col=colours.to.use[integer.traits])
...
Gen.sd <- function(web)
  # modified by JP 1 aug 17
  # () around sum(web)/length(web[,1]))
